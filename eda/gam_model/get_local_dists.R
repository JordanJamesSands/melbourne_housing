get_local_dists = function(lng,lat,osm_data,radius,dist_threshold,plot=F) {
    ...
    if(nrow(osm_locs)>1) {
        ...
        return(list('dists'=numeric(0),'plot'=NULL))
    }
    ...
    if(plot) {
        ...
        return(list('dists'=cluster_locs$dist,'plot'=plot))
    } else {
        return(list('dists'=cluster_locs$dist,'plot'=NULL))
    }
}