...
## Data Preparation {#data-preparation-link}
...
for more expensive properties. The log transform tames the 
...
modelling, so a decision was made to exclude values with `NA` entries in these fields
from the analysis. Imputation was attempted, but with no significant success.
...
those with 0 `land_area`, (mostly of `type` 'unit'), were not.
...
## Importing data from OSM {#show-data2-return-link}
...
entire feature space. Gradient boosted decision trees are used for their
robustness and successful track record. Finally, a generalised additive model is 
fitted, fitting splines to the feature-saleprice relationship, then fitting
...
The figure below shows the gain of each feature in the model after it is retrained
on all of `train0`.
...
The ensembling method proruced an error of `r ens_mod$best_score` on the
validation set.
