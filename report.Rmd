...
modelling, so a decision was made to exclude from the analysis sales with `NA` entries in those fields.
. Imputation was attempted, but with no significant success.
...
**3. Testing**<br>
    ...
    finally used to predicts on `test_data`.
...
of the test error. In the following models, features are selected to optimise the
...
hierarchical clustering was used. Distances between points were computed, and
geographical points clustered to lie within a circle of specified radius. These
clusters were then interpreted as the true landmarks/buildings.
...
clustering distance threshold is too great, many landmarks
...
bars, cafes and barbeques. Some of these were selected because of a suspected 
...
idea of centrality or population density. Public barbeques were included as a 
...
a leak from the target `log(price)` in the training data.
...
techniques. It is able to capture interactions and non-linear trends  without
explicit encoding. For more information on `xgboost`, you can view 
[the xgboost documentation](https://xgboost.readthedocs.io/en/latest/).
...
appropriately weigh each dimension of the data, so that the model is constant
...
trained with `xgboost`. Again, attention must be paid to the tickmarks along the X-axis, detailing the density of data
points.
...
other modelling options are explored. Other methods to
consider include neural networks and support vector machines.
