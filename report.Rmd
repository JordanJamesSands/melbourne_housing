...
and so a decision was made to exclude values with `NA` entries in these fields
from the analysis.
...
**1. Primary modelling**<br>
    ...
    `train0` by training on all but one fold and predicting on that fold. Then
    the models will be retrained on all of `train0` and predict on
    `ensemble_validation`.
...
alternative maintained by people all over the world making submissions. The 
...
radius, the other holds the distance to the nearest landmark. For example, 
...
fitted, fitting splines to the features and then taking a linear combination
...
a leak from the target `log(price)` in to the training data.
...
with the validation score is shown in orange, training in blue. The dashed lines
show one standard deviation above/below the mean training/validation score over the 
folds.
...
neighbouring data point is scaled by a kernel function, a decreasing function of 
...
The ensembling method was fitted with `r ens_mod$best_score`.
...
other modelling options are explored. The test error could be estimated
now by predicting on `test_data` but if later methods are constructed and
tested, the final model's test error would be underestimated, as the model
choice would be dependent on which has the lower test error. Other methods to
